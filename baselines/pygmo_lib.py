...
class MOPSO(object):
    ...
    def run(self):
        ...
        algo = pg.algorithm(pg.nspso(gen=self.termination, omega=0.3604, c1=0.8446, c2=0.2482, v_coeff=0.9121))
...
class MOEADGEN(object):
    ...
    def run(self):
        ...
        prob = pg.problem(
            PygmoAllocationProblem(self.df_pre_accuracy, self.df_true_accuracy, self.df_cost, self.model_list))
        pop = pg.population(prob, size=200)
        algo = pg.algorithm(pg.moead_gen(gen=self.termination, weight_generation='low discrepancy', decomposition='tchebycheff',
                                         neighbours=16))
